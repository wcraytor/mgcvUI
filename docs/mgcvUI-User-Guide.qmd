...
\item \textbf{\textcolor{norddark}{Quarto Reports}}
  \begin{itemize}
    \item Generate Quarto Report
    \item Convert Quarto Report
...
- **RCA computations (appraisal and market modes)** --- After fitting the model, mgcvUI can compute Residual Constraint Approach (RCA) output, which produces per-comparable adjustments, net/gross adjustment summaries, and an adjusted sale price for the subject property.
...
2. **Launch the application**: run `mgcvUI::mgcvUI()` in R. The app opens in your web browser on port 7880.
...
- **Factor columns**: Text, TRUE/FALSE, and R-factor columns are treated as categorical automatically — they can only enter the model as factors. A **numeric** column (e.g. a numeric area or style code) becomes a factor only when you tick its **Factor** checkbox; mgcvUI never infers categorical from a numeric's value range, so discrete numeric predictors like `bath_count` stay continuous (smoothed). The Special role is independent of the factor decision.
...
**7. Download Estimated Sale Prices \& Residuals** (titled *Download Estimated Target Variable(s) \& Residuals* in General mode) --- Exports predictions, residuals, CQA scores, and per-variable contributions as an Excel file to the project's output folder. Available in all purpose modes once the model is fit.
...
Variable selections and spline knots are **seeded, not locked** --- a note above the predictor table reminds you that every seeded Include/Linear/Factor selection remains editable. (Earth-detected interactions in the Allowed Interactions matrix are the exception: they stay checked and locked.)
...
\hline
Data & Interactive DataTable of imported data. In Appraisal and Market modes the tab is split into a Subject Property table (row 1) and a Comparable Sales table. \\
\hline
...
mgcvUI automatically saves your configuration to an SQLite database, keyed by the input filename. When you reload the same file, all settings are restored: target selection, predictor checkboxes, data types, mgcv parameters, and interaction matrix. The purpose mode itself is fixed by the active project, and each project remembers its last-used input file, which is reloaded automatically when the project is reopened.
...
- **RCA adjustments remain available** --- the "Calculate RCA Adjustments \& Download" step appears in market mode as well as appraisal mode.
...
- `weight` --- Observation weight column (only one allowed; rows with weight = 0 are excluded from fitting)

**Date \& Time Types:**

- `contract_date` --- Triggers automatic `sale_age` computation from the Effective Date
- `sale_age` --- Identifies a pre-computed sale-age column; when a `contract_date` column is also designated, the column is recomputed from the Effective Date
- `listing_date` --- Used as a fallback for computing Days on Market
- `dom` --- Identifies the Days on Market column

**Sale Types:**

- `sale_type` --- Identifies the sale-type column (e.g., arm's-length vs. REO/short sale)
...
Shows the imported data as an interactive DataTable with horizontal scrolling and 15 rows per page. In Appraisal and Market modes the tab is split into two tables --- **Subject Property** (row 1) and **Comparable Sales** (all remaining rows); in General mode a single table is shown.
...
- **Score type**: CQA or CQA per SF (if `living_area` designated). The per-SF option ranks residuals per square foot to reduce the impact of home size.
- **Subject CQA Score**: Numeric input accepting values from 0.00 to 10.00. The field starts blank; when an earthUI model has been imported, the score type and value entered in earthUI are carried forward from the shared project settings.

The **Generate** button stays disabled until a valid score (0.00--10.00) is entered. Click **Generate** to compute and download.
...
- **Other comps** (unchecked by default): Remaining comps, sorted by gross adjustment percentage (lowest first).
...
Report generation is a **two-step Quarto workflow**: first generate an editable \texttt{.qmd} report bundle, then convert it to the final document format(s).

## Generate Quarto Report

The **Generate Quarto Report** button (sidebar section 10, available once the model is fit) writes a self-contained Quarto report bundle to the active project's output folder (\texttt{<os>\_out\_mgcv}). The bundle contains:

- The \texttt{.qmd} report source (based on the package's \texttt{gam\_report.qmd} template)
- Pre-rendered plots (PNG and PDF) in a \texttt{plots/} subfolder
- A compact \texttt{report\_data.rds} payload with the tables and metrics
- A \texttt{reference.docx} used for Word styling

Because the bundle is plain Quarto source, you can edit the text, reorder sections, or combine it with other Quarto documents before rendering.

## Convert Quarto Report

The **Convert Quarto Report** section (sidebar section 11) renders a \texttt{.qmd} file via the \texttt{quarto} package. It defaults to the most recently generated bundle, but the **Browse** button lets you pick any \texttt{.qmd} file. Output format checkboxes offer:

- **HTML** --- Self-contained HTML document. Works on all platforms with no extra software. Recommended default.
- **Word (.docx)** --- Editable Word document, styled by the bundle's \texttt{reference.docx}.
- **PDF** --- Requires a LaTeX installation (see System Requirements below). If no LaTeX is detected, the PDF checkbox is hidden and a hint to run \texttt{tinytex::install\_tinytex()} is shown.

Rendered reports are saved next to the \texttt{.qmd} source in the project's output folder.

## Report Contents

The generated report includes:

- **Dataset \& Model**: response, predictors, observation count, family, smoothing method, and smooth-term count
- **Results Summary**: adjusted R$^2$, deviance explained, CV R$^2$ (when cross-validation was run), AIC, and BIC
- **Model Equation**: the fitted equation with $f_i$ notation for smooth terms and a smooth-function definitions list, aligned one term per line so long equations never run off the page
- **Smooth Terms** and **Parametric Terms** tables (EDF, F, p-value; Estimate, Std Error, t, p-value)
- **Enabled Interactions** matrix (only when the model contains tensor or factor-by-smooth interaction terms), rendered in landscape
- **Variable importance**: bar chart ranked by F-statistic (smooths) or $|t|$ (parametric)
- **Smooth Effects**: pre-rendered contribution plots for every model term, split into page-sized panels
- **Predictor Correlations** heatmap
- **Diagnostics**: residual diagnostics panel and an Actual vs Predicted scatter
- **mgcv Summary**: the raw \texttt{summary(model)} output
- **Notation legend**: an explanation of the value-contribution ($f_i$) notation with a citation to the Residual Constraint Approach (RCA) protocol

Result-tab material that is not part of the Quarto report (ANOVA, concurvity, sign check, basis check) remains available in the app's result tabs.

A white checkmark appears on the Generate Quarto Report button after the bundle is written successfully.
...
These exports are **programmatic** --- there is no Export Functions section in the app's interface. Use the exported package functions from the R console, for example:

```r
export_functions_zip(result, "gam_functions.zip",
                     languages = c("R", "Python", "C++", "SQLite"))
export_functions_sqlite(result, "gam_functions.sqlite")
```

The per-language generators (\texttt{generate\_r\_code()}, \texttt{generate\_python\_code()}, \texttt{generate\_cpp\_code()}) are also exported for direct use.
...
The file contains 1,488 rows --- the subject property in row 1 plus 1,487 residential sales --- with 39 columns, from a simulated MLS export. The data represents single-family home sales in a multi-area market with a range of property sizes, ages, and locations.
...
\hline
\texttt{sale\_type} & character & \texttt{sale\_type} & Sale-type code (\texttt{s} = subject row, \texttt{c} = comparable sale); imported from the \texttt{sale-type} header \\
\hline
...
\hline
\texttt{sale\_age} & numeric & \texttt{sale\_age} & Days from contract date to effective date \\
\hline
\texttt{coe\_date} & Date & --- & Close-of-escrow date \\
\hline
\texttt{listing\_status} & character & --- & Listing status (e.g., Sold) \\
\hline
\texttt{sale\_price} & numeric & (target) & Sale price --- response variable \\
\hline
\texttt{rent} & numeric & --- & Estimated monthly rent \\
\hline
\texttt{list\_price} & numeric & --- & Final list price \\
\hline
\texttt{original\_list\_price} & numeric & --- & Original list price \\
\hline
\texttt{ppsf} & character & --- & Price per square foot (formatted text) \\
\hline
...
\hline
\texttt{area\_text} & character & --- & Area / neighborhood name \\
\hline
\texttt{age} & numeric & \texttt{actual\_age} & Property age in years \\
\hline
\texttt{year\_built} & integer & --- & Year built \\
\hline
...
\hline
\texttt{latitude6} & numeric & --- & Latitude at 6-decimal precision \\
\hline
\texttt{longitude6} & numeric & --- & Longitude at 6-decimal precision \\
\hline
\texttt{garage\_spaces} & integer & --- & Number of garage bays \\
\hline
\texttt{fp\_count} & integer & --- & Fireplace count (sparse) \\
\hline
\texttt{no\_of\_stories} & integer & --- & Number of stories (sparse) \\
\hline
\texttt{style} & character & --- & Architectural style description \\
\hline
\texttt{view} & character & --- & View description \\
\hline
...
14. Generate the Quarto report bundle (Step 10) and convert it to HTML, Word, or PDF (Step 11)
...
\hline
valengrCore & Shared regProj project model, geo databases, and special-role logic (shared with earthUI and glmnetUI) \\
\hline
...
\hline
quarto, rmarkdown & Quarto report rendering (HTML/Word/PDF) \\
\hline
shinyFiles & Native file/folder browsers (regProj root, earthUI import, Quarto source) \\
\hline
readxl & Excel file import (CSV is read via base R) \\
\hline
RSQLite, DBI & Settings persistence and regProj databases (SQLite) \\
\hline
...
\hline
\textbf{writexl} & Excel downloads (predictions and RCA output). Prompted for on first use if missing. \\
\hline
\textbf{openxlsx} & Sales Comparison Grid workbook generation. \\
\hline
\textbf{callr} & Background model fitting with the progress overlay; without it, fitting runs synchronously. \\
\hline
\textbf{officer} & Programmatic Word export via \texttt{export\_gam\_docx()}. \\
\hline
\textbf{bslib} & Bootstrap 5 theming. \\
\hline
\end{tabularx}
\end{center}

...
Below the target selector, a scrollable table lists every remaining column with the following fields:
...
\hline
\texttt{Variable} & Column name (monospace font, truncated with ellipsis) \\
\hline
\texttt{Type} & Data type dropdown: numeric, integer, character, factor, logical, Date, POSIXct \\
\hline
\texttt{Inc} & Checkbox --- include this column as a predictor in the model \\
\hline
\texttt{Special} & Dropdown (appraisal/market only) --- see Special Column Types Reference below \\
\hline
\texttt{Factor} & Checkbox --- treat as a categorical factor (creates a separate coefficient per level) \\
\hline
\texttt{Linear} & Checkbox --- force a linear relationship (no smooth, just $\beta x$) \\
\hline
\texttt{NAs} & Count and percentage of missing values, color-coded: red ($\geq$50\%), orange ($\geq$20\%), grey ($>$0) \\
\hline
\end{tabularx}
\end{center}
...
## Contribution (Smooth Plots)

Interactive **plotly** smooth partial effect plots, one per univariate smooth term, each displayed in a bordered card. Each plot shows:
...
- **Word (.docx)** --- Formatted document built via the \texttt{officer} package. Suitable for editing and distribution.
- **PDF** --- Generated via \texttt{rmarkdown}. Paper size follows the locale setting (Letter or A4).
- **HTML** --- Self-contained HTML document via \texttt{rmarkdown}. Ideal for sharing electronically.
...
- Dataset description (file name, observations, response, predictors)
- Model specification (family, method, formula, gamma, select, basis, k)
- Results summary (R$^2$, CV R$^2$, Deviance Explained, AIC, n)
- Smooth terms table (EDF, F, p-value per term)
- Parametric terms table (Estimate, Std Error, t, p-value)
- Smooth partial effect plots (static ggplot2 renders)
- Diagnostic plots (residuals vs fitted, Q-Q, histogram, actual vs predicted)
